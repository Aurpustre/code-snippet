function main(boxList: string[], ballList: any[]) {
    ...
    function handleData() {
        ...
        if (!sign) {
            const existBox1 = boxList.filter(item => item[0] === ball0[0] && item[1] === ball1[0]);
            const existBox2 = boxList.filter(item => item[1] === ball0[0] && item[2] === ball1[0]);
            const existBox3 = boxList.filter(item => item[0] === ball0[0] && item[2] === ball1[0]);
            existBox = [...existBox1, ...existBox2, ...existBox3].sort((a: any, b: any) => a[4] - b[4]);
            if (existBox.length) {
                for (let i = 0; i < existBox.length; i++) {
                    const target = existBox[i];
                    const boxPaper = Number(target[4]);
                    let condTemp;
                    if (existBox1.includes(target)) {
                        condTemp = (dep1 === 0 || dep1 < startIndex) || (boxPaper === 1 && dep1 <= startIndex);
                    } else if (existBox2.includes(target)) {
                        condTemp = (dep1 === 0 || dep1 < startIndex) || (boxPaper === 2 && dep1 <= startIndex)
                    } else {
                        condTemp = (dep1 === 0 || dep1 < startIndex) || ((boxPaper === 1 || boxPaper === 2) && dep1 <= startIndex)
                    }
                    if (condTemp) {
